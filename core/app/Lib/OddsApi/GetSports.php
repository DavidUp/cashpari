...
class GetSports extends OddsApi {
    ...
    private function prepareNewLeague($sport, $categoryId) {
        return [
            ...
            'name'               => substr($sport->title, 0, 40),
            'short_name'         => substr($sport->title, 0, 40),
            'slug'               => createUniqueSlug($sport->key, League::class),
            'description'        => isset($sport->description) ? substr($sport->description, 0, 255) : null,
            'has_outrights'      => (int) $sport->has_outrights,
            'api_status'         => (int) $sport->active,
